...
#pragma omp parallel for collapse(3) private(i, j, m)
            ...
            for (y = 0; y < height; y += stride)
            {
                for (x = 0; x < width; x += stride)
                {
                    ...
                    for (i = 0; i < poolSize; i++)
                    {
                        ...
                        {
                            dtype value = input[sv::to1D(n, y + j, x + i, width, height)];
                            if (value > max)
                            {
                    ...
                    int global_idx = ((y / stride) * (width / stride) + (x / stride)) + n * (width / stride) * (height / stride);
                    out[global_idx] = max;
                }
            }
        }
